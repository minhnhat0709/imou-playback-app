  ...
  client.on('message', async (topic, message) => {
    ...
    if (parts.length === 2 && parts[0].trim().toLowerCase() === 'drop') {
      ...
      try {
        ...
        console.log('[Imou] Waiting 15 seconds for camera to write the video file to SD card...');
        await new Promise(r => setTimeout(r, 15000));

        let beginTime, endTime;
        try {
          console.log(`[Imou] Querying camera SD card records around event time...`);
          // Query records from 60 seconds before to 60 seconds after the event
          const queryStart = new Date(eventTime - 60000);
          const queryEnd = new Date(eventTime + 60000);
          const qStartStr = formatDate(queryStart);
          const qEndStr = formatDate(queryEnd);

          const records = await queryLocalRecords(deviceId, qStartStr, qEndStr);
          console.log(`[Imou] Found ${records.length} records on SD card around event window.`);

          // Find the record that overlaps [eventTime - 7000, eventTime + 7000]
          const targetStart = eventTime - 7000;
          const targetEnd = eventTime + 7000;

          let bestRecord = null;
          for (const rec of records) {
            const recStart = new Date(rec.beginTime).getTime();
            const recEnd = new Date(rec.endTime).getTime();

            // Check overlap
            if (recStart <= targetEnd && recEnd >= targetStart) {
              bestRecord = rec;
              break;
            }
          }
          ...
          if (bestRecord) {
            beginTime = bestRecord.beginTime;
            endTime = bestRecord.endTime;
            console.log(`[Imou] Aligned with matching SD card record: ${bestRecord.recordId} (${beginTime} to ${endTime})`);
          } else {
            console.log(`[Imou] No overlapping SD card record found. Using calculated time window.`);
            beginTime = formatDate(new Date(eventTime - 7000));
            endTime = formatDate(new Date(eventTime + 7000));
          }
        } catch (queryErr) {
          console.error(`[Imou] Failed to query local records list, falling back to calculated time window:`, queryErr.message);
          beginTime = formatDate(new Date(eventTime - 7000));
          endTime = formatDate(new Date(eventTime + 7000));
        }
